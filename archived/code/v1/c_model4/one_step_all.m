...
function [ cost ] = one_step_all(x, v, s, number, delta_t)
cost = 0;
for i=2:(number-1)
    % Step One
    % one_step_middle(x, v, v_front, v_behind, d_front, delta_t)
    cost = cost + one_step_middle(x(i), x(i-1), x(i+1), v(i), v(i-1), v(i+1), s(i-1)-s(i), delta_t );
end
% For the last car
i = i+1;
% one_step_last(x, v, v_front, d_front, delta_t)
cost = cost + one_step_last(x(i),  x(i-1), v(i), v(i-1), s(i-1)-s(i), delta_t);
