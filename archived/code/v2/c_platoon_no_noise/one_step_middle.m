...
function [ target ] = one_step_middle(a, a_front, a_behind, v, v_front, v_behind, d_front, TotalS, TotalF, delta_t)
v_next = v + a*delta_t;
...
%target = w1*(TotalF+fuel)/(TotalS+s) + w2*(R_e^2) + w3*((v_next - v_front_next)^2) + w4*(a^2) + w5*((v_next - v_behind_next)^2) + w6*(R_b^2);
