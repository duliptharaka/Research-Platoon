function [ target ] = one_step_last(a, a_front, v, v_front, d_front, TotalS, TotalF, delta_t)
...
% Cost function
%target = w1*fuel/s + w2*(R_e^2) + w3*((v_next - v_front_next)^2) + w4*(a^2) + w6*(R_b^2);
%target = w1*(TotalF + fuel)/(TotalS + s) + w2*(R_e^2) + w3*((v_next - v_front_next)^2) + w4*(a^2) + w6*(R_b^2);
target = w1*fuel/s + w2*(R_e^2) + 200*((d_front_next+700)^0.5)+5*((d_front_next+700)^2)+ w4*(a^2) + w3*((v_next - v_front_next)^2)+ 1000*abs(v_front_next-v_next);
