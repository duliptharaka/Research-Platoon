...
color = {'k','r','b','g','y','m','c', [1,0.5,0],[0.7,0.5,1]}; 
%color = ['k','r','b','g','y','m','c', 'rg','by']; 
for it = 3:2:9
    number = it;
    % load result
    load(fullfile(strcat(pwd, '/result'), strcat('output_a_',num2str(number), '.mat')));
    load(fullfile(strcat(pwd, '/result'), strcat('output_s_',num2str(number), '.mat')));
    load(fullfile(strcat(pwd, '/result'), strcat('output_v_',num2str(number), '.mat')));
    ...
    for j = 1:(number-1)
        for i=1:step
            time(i) = 1*(i-1);
            dist(i,j) = output_s(i,j) - output_s(i,j+1);
        end
    end
    % Figure 1: acceleration
    figure
    subplot(3,2,1)
    plot(time(1:step),output_a(:,1), color{1});     
    hold on   
    ind = 2;
    while ind ~= number+1
        plot(time(1:step),output_a(:,ind), 'Color', color{ind});
        ind = ind+1;
    end
    hold off
    xlabel('time [s]')
...
%     set(xlabh,'Position',get(xlabh,'Position') + [0 0.65 0])
    ...
    % Figure 2: veloity
    subplot(3,2,2)
    plot(time,output_v(:,1), color{1});
    hold on
    ind = 2;
    while ind ~= number+1
        plot(time,output_v(:,ind), 'Color', color{ind});
        ind = ind+1;
    end
    hold off    
    xlabel('time [s]')
...
%     set(xlabh,'Position',get(xlabh,'Position') + [-50 5 0])
    ...
    % Figure 3: positions
    subplot(3,2,3)
    plot(time,output_s(:,1), color{1});
    hold on
    ind = 2;
    while ind ~= number+1
        plot(time,output_s(:,ind), 'Color', color{ind});
        ind = ind+1;
    end
    hold off     
    xlabel('time [s]')
...
%     xlabh = get(gca,'XLabel');
%     y_aix = get(xlabh,'Position')+ [-50 1.4 0]
%     set(xlabh,'Position',get(xlabh,'Position') + [-50 1.5 0])
...
%     suptitle(['Results of the '  num2str(number)  '-car model in the HWFET driving cycle (Centralized Control)'])
    saveas(gca, fullfile(strcat(pwd,'/figure/result_', num2str(number),'_c.fig')), 'fig')     
        figure
    if number == 3
        subplot(2,1,1)
    ...
    saveas(gca, fullfile(strcat(pwd,'/figure/cost_', num2str(number),'_c.fig')), 'fig')       
end
