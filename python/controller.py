def optimal_a(delta_d, target_v, target_a, delta_t):
    """
    ...
    min(w1(D-0.5a*dt)^2+w2(V-a*dt)^2+w3(A-a)^2)
    (w1(D-0.5a*dt)^2+w2(V-a*dt)^2+w3(A-a)^2)d(a) = 0
    (w1*D^2-w1*D*dt*a+0.25*w1*dt^2*a^2
    +w2*V^2-2*w2*V*dt*a+w2*dt^2*a^2
    +w3*A^2-2*w3*A*a+w3*a^2)d(a)=0
    -w1*D*dt+0.5*w1*dt^2*a
    -2*w2*V*dt+2*w2*dt^2*a
    -2*w3*A+2*w3*a = 0
    a = (2*w1*D*dt+4*w2*V*dt+4*w3*A)/(w1*dt^2+4*w2*dt^2+4*w3)
    """
    w1 = 0.5  # D
    w2 = 0.3  # V
    w3 = 0.2  # A
    return (2*w1*delta_d*delta_t+4*w2*target_v*delta_t+4*w3*target_a)/(w1*delta_t**2+4*w2*delta_t**2+4*w3)
...
class DecentralizedController(Controller):
    ...
    def update(self):
        _delta_d = self.front_car_status.d + self.front_car_status.v * self.delta_t + 0.5 * self.front_car_status.a * self.delta_t - \
            self.safty_dist - \
        ...
        self.desired_action.d = self.system_status.d + \
            self.system_status.v * self.delta_t + 0.5 * self.desired_action.a * self.delta_t
...
class CentralizedController(Controller):
    ...
    def update(self):
        _delta_d = self.front_car_status.d + self.front_car_status.v * self.delta_t - \
