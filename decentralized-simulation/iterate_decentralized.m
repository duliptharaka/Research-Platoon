...
num_trials = 1;
num_steps = 10;
if_plot_conclusion = 0;
if_plot_example_run = 1;
step_v = 0.05;
...
sigma_m = 0.00;
    ...
    for step_idx = 1:num_steps
        ...
        for idx=1:num_trials
            car_results = decentralized_platoon(sigma_a, sigma_m, SF, if_plot_example_run);
    ...
    if if_plot_conclusion
        if parameter == 1
            file_name=sprintf('dec_sigma_a');
            x_label = 'Actuator Noises Sigma';
        elseif parameter == 2
            file_name=sprintf('dec_sigma_m');
